    ...
    for i, (old_folder, old_index, new_index) in enumerate(episode_mapping):
        ...
        if source_path:
            try:
                ...
                for feature, target_dim in [("observation.state", OBS_STATE_DIM), ("action", ACTION_DIM)]:
                    if feature in df.columns:
                        def pad_to_dim(x):
                            ...
                            if len(arr) < target_dim:
                                return np.pad(arr, (0, target_dim - len(arr)), 'constant').tolist()
                            elif len(arr) > target_dim:
                                return arr[:target_dim].tolist()
                            else:
                                return arr.tolist()
                        df[feature] = df[feature].apply(pad_to_dim)
...
def merge_datasets(
                                ...
                                if stat_type in stats["stats"][feature]:
                                    ...
                                    if isinstance(values, list):
                                        if len(values) < target_dim:
                                            ...
                                            stats["stats"][feature][stat_type] = values[:target_dim]
        ...
        if all_stats_data:
            ...
            for feature in merged_stats:
                if feature in all_stats_data[0]:
                    ...
                    # Use fixed target dims for state and action
                    if feature == "observation.state":
                        target_dim = OBS_STATE_DIM
                    elif feature == "action":
                    ...
                    if "min" in merged_stats[feature] and all("min" in stat for stat in values):
                        ...
                        for val in values:
                            ...
                            if len(val_flat) < target_dim:
                                padded = np.zeros(target_dim)
                                padded[: len(val_flat)] = val_flat
                                padded_mins.append(padded)
                            else:
                                padded_mins.append(val_flat[:target_dim])
                    ...
                    if "max" in merged_stats[feature] and all("max" in stat for stat in values):
                        ...
                        for val in values:
                            ...
                            if len(val_flat) < target_dim:
                                ...
                                padded_maxs.append(val_flat[:target_dim])
                    ...
                    if "mean" in merged_stats[feature] and all("mean" in stat for stat in values):
                        ...
                        for val in values:
                            ...
                            if len(val_flat) < target_dim:
                                ...
                                padded_means.append(val_flat[:target_dim])
                    ...
                    if "std" in merged_stats[feature] and all("std" in stat for stat in values):
                        ...
                        for val in values:
                            ...
                            if len(val_flat) < target_dim:
                                ...
                                padded_stds.append(val_flat[:target_dim])
                        if all("count" in stat for stat in values):
                            counts = [stat["count"][0] for stat in values]
                            total_count = sum(counts)
    ...
    if "features" in info:
        ...
        if "observation.state" in info["features"]:
            info["features"]["observation.state"]["shape"] = [OBS_STATE_DIM]
